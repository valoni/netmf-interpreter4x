...
namespace microFramework.Driver
{
    ...
    public class ST7735
    {
        ...
        private readonly OutputPort _controlPin;
        private readonly OutputPort _resetPin;
        private readonly OutputPort _backlightPin;
        private readonly OutputPort _chipSelectPin;
        ...
        public ST7735(Cpu.Pin resetPin, Cpu.Pin backlightPin, Cpu.Pin controlPin, SPI.SPI_module spiLine, Cpu.Pin chipSelectPin)
        {
            _buffer1 = new byte[1];
            _buffer2 = new byte[2];
            _buffer4 = new byte[4];
            ...
            _resetPin = new OutputPort(resetPin, true);
            _backlightPin = new OutputPort(backlightPin, true);
            _controlPin = new OutputPort(controlPin, true);
            _chipSelectPin = new OutputPort(chipSelectPin, true);

            _backlightPin.Write(false);
            //_chipSelectPin.Write(true);

            SPI.Configuration spiConfig = new SPI.Configuration(
                ChipSelect_Port: chipSelectPin,         // Chip select is digital IO 4.
                ...
                Clock_RateKHz: 12000,                   // 12MHz clock speed.
                SPI_mod: spiLine                        // Use SPI1
            );
        ...
        public void DrawImage(byte[] data)
        {
            ...
            if (data.Length == 0) throw new ArgumentException("data.Length must not be zero.", (data).ToString());
        ...
        public void DrawImage(int x, int y, Image image)
        {
            ...
            if (x < 0) throw new ArgumentOutOfRangeException((x).ToString(), "x must not be negative.");
            if (y < 0) throw new ArgumentOutOfRangeException((y).ToString(), "y must not be negative.");
        ...
        public void DrawFilledRectangle(int x, int y, int width, int height, Color color)
        {
            if (x < 0) throw new ArgumentOutOfRangeException((x).ToString(), "x must not be negative.");
            if (y < 0) throw new ArgumentOutOfRangeException((y).ToString(), "y must not be negative.");
            if (width < 0) throw new ArgumentOutOfRangeException((width).ToString(), "width must not be negative.");
            if (height < 0) throw new ArgumentOutOfRangeException((height).ToString(), "height must not be negative.");
        ...
        {
            if (x0 < 0) throw new ArgumentOutOfRangeException((x0).ToString(), "x0 must not be negative.");
            if (y0 < 0) throw new ArgumentOutOfRangeException((y0).ToString(), "y0 must not be negative.");
            if (x1 < 0) throw new ArgumentOutOfRangeException((x1).ToString(), "x1 must not be negative.");
            if (y1 < 0) throw new ArgumentOutOfRangeException((y1).ToString(), "y1 must not be negative.");

            var steep = System.Math.Abs(y1 - y0) > System.Math.Abs(x1 - x0);
        ...
        public void DrawCircle(int x, int y, int r, Color color)
        {
            if (x < 0) throw new ArgumentOutOfRangeException((x).ToString(), "x must not be negative.");
            if (y < 0) throw new ArgumentOutOfRangeException((y).ToString(), "y must not be negative.");
            if (r <= 0) throw new ArgumentOutOfRangeException((r).ToString(), "radius must be positive.");
        ...
        {
            if (letter > 126 || letter < 32) throw new ArgumentOutOfRangeException((letter).ToString(), "This letter cannot be drawn.");
            if (x < 0) throw new ArgumentOutOfRangeException((x).ToString(), "x must not be negative.");
            if (y < 0) throw new ArgumentOutOfRangeException((y).ToString(), "y must not be negative.");
    ...
    public class Image
    {
        ...
        public void SetPixel(int x, int y, Color color)
        {
            if (x < 0) throw new ArgumentOutOfRangeException((x).ToString(), "x must not be negative.");
            if (y < 0) throw new ArgumentOutOfRangeException((y).ToString(), "y must not be negative.");
            if (x > Width) throw new ArgumentOutOfRangeException((x).ToString(), "x must not exceed Width.");
            if (y > Height) throw new ArgumentOutOfRangeException((y).ToString(), "y must not exceed Height.");
